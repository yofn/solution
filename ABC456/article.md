...
- 若 $S[i] \neq S[i-1]$，这些子串都可以向后延伸一位，再加上"只选 $S[i]$ 自己"这一个新子串
...
因此定义二维状态：$dp[i][j]$ 表示前 $i$ 个字符中，以字符 $j$（$0$ 代表 `a`，$1$ 代表 `b`，$2$ 代表 `c`）结尾的合法子序列个数。
...
| 题目 | 模型 | 状态设计 | 维度 |
|------|------|----------|------|
| C 题 | 子串（连续） | $f[i]$ = 以位置 $i$ 结尾的个数 | 一维 |
| D 题 | 子序列（可不连续） | $dp[i][j]$ = 以字符 $j$ 结尾的个数 | 二维（滚动后 $O(1)$ 空间）|

同样的"不相邻"限制，仅仅因为<span style="color:#e74c3c">连续性</span>的差异，就让状态从一维跳到了二维。这就是 DP 状态设计的魅力所在。
