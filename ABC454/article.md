...
这意味着，如果 A 能变成 B，那么 B 也能变成 A。进一步想：所有能互相转换的字符串，其实都属于同一个"等价类"。我们只要找到每个等价类的<span style="color:#2980b9">唯一代表元</span>（规范形式），然后比较 A 和 B 的代表元是否相同即可。

什么是最简的规范形式？就是<span style="color:#e74c3c">字符串中不再含有 `(xx)` 子串</span>的形式。因为只要有 `(xx)`，就可以用第一种操作把它缩短成 `xx`。

如何从任意字符串得到规范形式？从左到右扫描，用一个栈维护当前结果：
