...
CDN_BASE = "https://cdn.jsdelivr.net/gh/yofn/solution@main/ABC455/figures_sel"
...
def md_to_html(md_path: str, html_path: str):
    ...
    # Mapping from section title to code image
    # We detect which problem we're in by tracking h2 headers
    problem_letter = None
    ...
    for line in lines:
        ...
        if stripped.startswith("```"):
            if not in_code_block:
                ...
                code_buffer = []
                # Determine which code image to use based on current problem
            else:
                in_code_block = False
                # Emit code image
        ...
        if in_code_block:
            # Skip code content in HTML (replaced by image)
            continue
